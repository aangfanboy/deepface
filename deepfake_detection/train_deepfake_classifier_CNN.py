...
class MainModel(MMA.MainModel):
    ...
    def __call__(self, input_shape, weights: str = None, arcface_model_path: str = None, deepfake_model_path: str = None, num_classes: int = 10,
        ...
        if deepfake_model_path is not None and os.path.exists(deepfake_model_path):
            self.model = tf.keras.models.load_model(deepfake_model_path, custom_objects={"ArcFaceLayer": MMA.ArcFaceLayer})
            ...
            x = MMA.BatchNormalization(momentum=0.9, scale=False, epsilon=2e-5, name="bn_deepfake_sub")(x1)

            label_input_layer = tf.keras.layers.Input((None,), dtype=tf.int64)
            if use_arcface:
                x = MMA.ArcFaceLayer(num_classes=num_classes, arc_m=0.5, arc_s=64., regularizer_l=regularizer_l,
...
class Trainer:
    ...
    def get_wrong(y_real, y_pred):
        return tf.where(tf.cast(tf.equal(tf.argmax(tf.nn.softmax(y_pred), -1), tf.cast(y_real, tf.int64)), tf.float32) == 00)
    ...
    def __init__(self, model_engine: MMA, dataset_engine: DSM, tensorboard_engine: TBH, use_arcface: bool,
        ...
        self.model_engine(
            ...
            deepfake_model_path=self.model_path,  # path of keras model(h5) , it is okay if doesn't exists
...
if __name__ == '__main__':
    ...
    trainer = Trainer(
        ...
        model_path="deepfake_model.h5",  # it will save only weights, you can chose "h5" as extension too
    ...
    trainer(max_iteration=5000, alfa_step=5000, qin=0.2)
